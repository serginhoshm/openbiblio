 ...
 */
  ...
  if ($_REQUEST['type'] == 'previous') {
    ...
    if (!empty($errs)) {
      ...
      header('Location: ../reports/report_criteria.php?msg='.U('Parâmetros incorretos, verifique:'));
      exit();
    }
  }
  ...
  if ($rpt->count() == 0) {
    echo $loc->getText("Nenhum resultado encontrado.");
    require_once("../shared/footer.php");
    exit();
  }

  echo '<p>'.$rpt->count().' Resultado encontrado.</p>';
  if ($format == 'paged') {
...
<table class="resultshead">
  <tr>
      <th><?php echo $loc->getText("Resultado do relatório:"); ?></th>
